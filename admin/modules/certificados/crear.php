<?php
/**
 * Technical del Perú — Panel de Certificados (Crear / Emitir)
 * 
 * Permite a los usuarios autorizados (Administrador y Vendedor)
 * emitir y registrar un nuevo certificado en el sistema.
 * Genera el código único de validación y la imagen QR física.
 * 
 * @version 1.0.0
 */
...
// Variables para rellenar el formulario en caso de error
$ruc = '';
$razonSocial = '';
$participante = '';
$tipoCertificado = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty($ruc) || empty($razonSocial) || empty($participante) || empty($tipoCertificado) || empty($fechaEmision)) {
            ...
            if ($id) {
                setFlash('success', '¡Excelente! El certificado ha sido emitido con éxito y su código QR de seguridad fue generado automáticamente.');
                redirect(ADMIN_URL . '/modules/certificados/index.php');
            } else {
                $error = 'Ocurrió un error al guardar el certificado en la base de datos. Verifique los registros del sistema.';
            }
        }
    } catch (Exception $e) {
        $error = 'Ocurrió un error inesperado al procesar la solicitud.';
        error_log('Error al crear certificado en admin: ' . $e->getMessage());
    }
}
...
<!-- ============================================================= -->
<div class="glass-card rounded-2xl p-6 sm:p-8 border border-white/5 shadow-2xl relative overflow-hidden max-w-3xl mx-auto" id="create-cert-container">
    ...
    <div class="border-b border-white/5 pb-4 mb-6">
        <h2 class="text-lg font-['Outfit'] font-bold text-white">Emitir Nuevo Certificado</h2>
        <p class="text-xs text-gray-400 mt-0.5">Rellene los campos obligatorios para registrar la competencia técnica y generar el código QR.</p>
    </div>

    <!-- Alert para errores -->
    <?php if (!empty($error)): ?>
        <div class="bg-red-500/10 border border-red-500/30 text-red-300 px-4 py-3 rounded-xl text-sm flex items-start gap-2.5 mb-6 animate-pulse" role="alert" id="create-error-alert">
    ...
    <form action="" method="POST" class="space-y-6" id="create-certificado-form" onsubmit="return validateForm()">
        <!-- CSRF Token Field -->
        <?php echo csrfField(); ?>
        ...
        <!-- Tipo de Certificado con sugerencias o autocompletado en el placeholder -->
        <div class="space-y-1.5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <!-- Estado -->
            <div class="space-y-1.5">
                <label for="estado" class="text-xs font-semibold text-gray-300 uppercase tracking-wider block">Estado Inicial</label>
